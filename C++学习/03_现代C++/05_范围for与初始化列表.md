...
> ⚠️ **黄金法则**：范围 for 默认写 `const auto&`（只读不拷贝）；需要修改原容器时写 `auto&`；只有基本类型小对象才用 `auto`（值拷贝）。
>
> | 写法 | 场景 | 示例 |
> |------|------|------|
> | `const auto&` | **默认推荐**：只读遍历，零拷贝 | `for (const auto& s : vec_of_string)` |
> | `auto&` | 需要修改原元素 | `for (auto& x : vec) { x *= 2; }` |
> | `auto` | 基本类型小对象（int, char, bool） | `for (auto x : {1, 2, 3})` |
